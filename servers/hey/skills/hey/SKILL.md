...
1. Prefer the **`paragraphs`** argument: an array of strings. MadCP joins them and converts to HTML (`<div>…</div>`) so HEY/Action Text keeps the breaks (plain `\n` alone is collapsed by HEY).
