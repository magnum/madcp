  ...
  class Config
    ...
    def initialize(root:)
      ...
      @app_auth = AppAuth.new(
        tokens_path: ENV.fetch("MADCP_AUTH_TOKENS_PATH", File.join(@root, "data", "auth_tokens")),
        users_path: ENV.fetch("MADCP_AUTH_USERS_PATH", File.join(@root, "data", "auth_users")),
        secret: Madcp.sanitize_env_value(ENV.fetch("MADCP_SECRET_KEY", ENV.fetch("SECRET_KEY", ""))),
